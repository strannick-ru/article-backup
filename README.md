...
При запуске через `run-docker.sh` симлинк не используется: `site/content` готовится
как обычный каталог, в который Docker монтирует каталог бэкапа. Hugo игнорирует
`content`, если это симлинк за пределы корня проекта, и собирает сайт без статей.
