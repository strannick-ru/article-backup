...
- Генерируется `site/hugo.toml` из секции `hugo:` в конфиге, включая `[[module.mounts]]`
  с путём до каталога бэкапа — через него Hugo читает статьи
- Создаётся симлинк `site/content → output_dir`, который используется при дедупликации assets

Сам по себе симлинк Hugo не читает: `content` за пределами корня проекта игнорируется,
и сайт собирается без статей. При запуске через `run-docker.sh` симлинк не нужен —
`site/content` готовится как обычный каталог, в который Docker монтирует бэкап.
...
### Экономия места для медиафайлов

`site/build.sh` и `run-docker.sh` после успешной сборки заменяют совпадающие копии
медиафайлов в `site/public` hardlink-ссылками на файлы из каталога бэкапа.
URL и работа nginx при этом не меняются.

Оптимизация работает, только если каталог бэкапа и `site/public` находятся на одной
файловой системе. В остальных случаях сайт остаётся самодостаточным, но файлы
занимают место как обычные копии. Перед каждой сборкой `site/public` удаляется,
поэтому изменять файлы внутри него вручную не следует.

Проверить hardlink можно по совпадающим inode:

```bash
stat backup/boosty/author/posts/slug/assets/file.mp4 \
     site/public/boosty/author/posts/slug/assets/file.mp4
```
