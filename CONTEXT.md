...
9. Sponsr: HTML предобрабатывается перед html2text — пустые теги форматирования удаляются (`decompose`), а теги содержащие только пробелы заменяются на текстовое содержимое (`replace_with`), чтобы не терять пробелы между элементами. Затем постобработка: склеивание вложенных `** _текст_**` в `***текст***`, удаление bidi-маркеров, нормализация пробелов вокруг Unicode-кавычек, перемещение форматирования внутрь ссылок.
