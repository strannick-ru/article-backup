...
9. Sponsr: HTML предобрабатывается перед html2text:
    - удаляются пустые теги форматирования `b/strong/em/i`, если внутри нет других тегов и текст пуст/whitespace
    - узкая структурная нормализация только для паттерна:
      `<em>LEFT</em><a ...><em>MID</em></a><em>RIGHT</em>` -> `<em>LEFT <a ...>MID</a> RIGHT</em>`
      с guard'ами: без вложенных тегов, одинаковые атрибуты у `em`, между соседними узлами только whitespace
      (пробелы/табы рядом со ссылкой фиксируются через NBSP (`\u00a0`) — наличие пробела сохраняется, но серия
      пробелов может схлопнуться в один NBSP; если пробела нет, но он нужен между словами — вставляется NBSP;
      после html2text NBSP/NNBSP нормализуются обратно в обычный пробел)
    - затем rule-based постобработка Markdown:
      - удаление bidi-маркеров; нормализация NBSP/NNBSP (`\u00a0`, `\u202f`) в обычный пробел
      - убираем лишние пробелы, добавленные html2text рядом с Unicode-кавычками («» „“ ‘’)
      - склейка артефактов html2text для вложенных `strong/em` в `***...***` (`** _x_**`, `_**x**_`)
      - перенос форматирования внутрь ссылок (варианты `[** _x_**](u)`, `***[x](u)***`, `_[x](u)_`)
      - `_cleanup_spacing`: трим пробелов внутри `[]` у ссылок с форматированием; убирает пробелы перед пунктуацией
        сразу после emphasis; чинит пропущенный пробел перед `**bold**` внутри `_italic_`; консервативно склеивает
        разрывы *внутри слова* вокруг emphasis; добавляет пробел после запятой перед ссылкой; разлипляет
        "слово+ссылка" и "ссылка+слово"
    - правило для внутрисловного `_..._`: при совпадении эвристик конвертируем в `*...*` (CommonMark/Goldmark),
      с защитой от литералов вида `foo_bar_baz`
    - регрессии фиксировать тестами (`tests/test_sponsr_normalize.py`)
