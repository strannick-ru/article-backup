...
9. Sponsr: HTML предобрабатывается перед html2text — удаляются пустые теги форматирования. Затем постобработка: склеивание вложенных `** _текст_**` в `***текст***`, удаление bidi-маркеров, нормализация пробелов вокруг Unicode-кавычек, перемещение форматирования внутрь ссылок.
