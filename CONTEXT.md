...
9. Конвертер html2text для Sponsr добавляет лишние пробелы вокруг кавычек-ёлочек («...») и другой Unicode-пунктуации при обработке `<em>` и `<strong>`. Решается постобработкой регулярными выражениями.
