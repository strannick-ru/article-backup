...
9. Конвертер html2text для Sponsr добавляет лишние пробелы вокруг кавычек-ёлочек («...») и другой Unicode-пунктуации, а также ломает вложенные `<em>/<strong>` (появляются разорванные `**` и bidi-маркеры). Решается постобработкой регулярными выражениями.
