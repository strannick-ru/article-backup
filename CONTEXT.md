...
- Hugo (проверено на v0.166.0) не заходит в `content`, если это симлинк за пределы корня проекта: сборка завершается успешно, но статей в `public` нет. Поэтому `run-docker.sh` удаляет симлинк и готовит `site/content` как обычный каталог — точку монтирования бэкапа. Для локальной сборки `backup.py` пишет в `hugo.toml` секцию `[[module.mounts]]` с относительным путём до `output_dir`; в Docker (задан `BACKUP_OUTPUT_DIR`) mount не генерируется. Симлинк `site/content` остаётся ради `site/build.sh`, который передаёт путь `content` в дедупликацию; Hugo его игнорирует, дублирования страниц нет.
- Compose-команды Hugo не должны завершаться ненулевым кодом при отсутствии каталога одной из платформ, иначе `set -e` в `run-docker.sh` обрывает запуск до дедупликации assets
...
- После успешной Hugo-сборки `site/deduplicate-assets.sh` заменяет идентичные public-копии assets hardlinks на исходные файлы бэкапа. Перед каждой сборкой `site/public` полностью очищается, чтобы Hugo не мог записать поверх hardlink. В Docker дедупликация выполняется на хосте из `run-docker.sh`; при разных файловых системах остаются обычные копии.
